...
#define PRLCKWAIT	'\011'		/* process is waiting on a lock */
...
struct	pentry	{
	...
	int	pwaitret;
	int 	plockwaitret;		/* is process waiting on a lock	*/
	int 	plock;		/* process is blocked on this lock Id */
	int 	plocktype;		/* process type for which the lock waits */
	int 	plocktime;		/* time at which waiting for lock starts*/
	int 	pinh;		/* Inherited priority of the process */
	long long  plockbitmask; /* bit mask to store all the locks held by this process*/ 
};
